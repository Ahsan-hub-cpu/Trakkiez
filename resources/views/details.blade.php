        ...
        <div class="product-single__price">
          <div class="price-display">
            @if($product->sale_price)
              <div class="price-row">
                <span class="original-price">PKR {{ $product->regular_price }}</span>
                <span class="sale-price">PKR {{ $product->sale_price }}</span>
              </div>
              <div class="discount-badge">
                Save PKR {{ $product->regular_price - $product->sale_price }}
              </div>
            @else
              <div class="price-row">
                <span class="current-price">PKR {{ $product->regular_price }}</span>
              </div>
            @endif
          </div>
        </div>
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    @endphp
                      @foreach($availableColors as $colorId => $variations)
                        ...
                        @endphp
                      <button type="button" 
                          class="color-btn {{ $totalStock <= 0 ? 'sold-out' : '' }}"
                          data-color-id="{{ $color->id }}"
                          data-color-name="{{ $color->name }}"
                          data-color-code="{{ $color->code }}"
                          data-total-stock="{{ $totalStock }}"
                          {{ $totalStock <= 0 ? 'disabled' : '' }}
                          style="{{ $color->code ? 'background-color: ' . $color->code . ';' : '' }}"
                          title="{{ $color->name }}{{ $totalStock <= 0 ? ' (Sold Out)' : '' }}">
                      </button>
                    @endforeach
                    </div>
...
function toggleDescription() {
    ...
    if (btn.classList.contains('expanded')) {
        // Collapse
        preview.textContent = `{{ \Illuminate\Support\Str::limit($product->description, 150) }}`;
