                        ...
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                ...
                                @foreach(explode(',', $product->images) as $gimg)
                                    <div class="swiper-slide product-single__image-item">
                                        <img loading="lazy" class="h-auto" src="{{ asset('uploads/products/' . trim($gimg)) }}" width="674" height="674" alt="{{ $product->name }}" />
                    ...
                    <div class="product-single__thumbnail">
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                ...
                                @foreach(explode(',', $product->images) as $gimg)
                                    <div class="swiper-slide product-single__image-item">
                                        <img loading="lazy" class="h-auto" src="{{ asset('uploads/products/' . trim($gimg)) }}" width="104" height="104" alt="{{ $product->name }}" />
                                    </div>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div class="swiper-slide product-card" style="position: relative;">
                                <div class="pc__img-wrapper">
                                    <a href="{{ route('shop.product.details', ['product_slug' => $rproduct->slug]) }}">
                                        ...
                                        @if($rproduct->hover_image && $rproduct->hover_image !== 'default-hover.jpg')
                                            <img loading="lazy" src="{{ asset('uploads/products/' . $rproduct->hover_image) }}" width="330" height="400" alt="{{ $rproduct->name }}" class="pc__img pc__img-second">
                                        @endif
                                    </a>
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    quantityInput.val(1);
                    qtyError.text('');
                    cartStatus.text('Item added to cart').show().delay(3000).fadeOut();
                } else {
    ...
    $('.pc__atc').on('click', function(e) {
        ...
        const form = button.closest('form[name="addtocart-form"]');
        const formData = form.serialize();
        $.ajax({
            url: form.attr('action'),
            method: 'POST',
            ...
            success: function(response) {
                console.log('Related product add to cart response:', response);
                if (response.success) {
                    window.updateCartCount();
                    window.loadCartContent();
                    $('#cartModal').modal('show');
                    button.removeClass('btn-primary').addClass('btn-warning').text('Go to Cart');
            ...
            error: function(xhr) {
                console.error('Related product add to cart error:', xhr.responseText);
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: xhr.responseJSON?.message || 'An error occurred while adding to cart'
                });
            }
        });
    });
    ...
    function updateCartStatus(sizeId) {
        $.ajax({
            ...
            success: function(response) {
                console.log('Cart status response:', response);
            ...
            error: function(xhr) {
                console.error('Cart status error:', xhr.responseText);
                cartStatus.text('').hide();
            }
        });
    }
    ...
    productCards.forEach(card => {
        ...
        const hoverImg = card.querySelector('.pc__img-second');
        if (hoverImg) {
